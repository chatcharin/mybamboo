...
buffer contoaddressvariable(int value) {
    buffer buf;
    sprintf(buf.buff, "-%04X", value);
    return buf;
}
...
HEX initiantVariable(char operand[]) {
    HEX bufs;
    switch (operand[0]) {
        case '0':deci(operand);
            break;
        case '1':bufs=deci(operand);
            break;
        case '2':bufs=deci(operand);
            break;
        case '3':bufs=deci(operand);
            break;
        case '4':bufs=deci(operand);
            break;
        case '5':bufs=deci(operand);
            break;
        case '6':bufs=deci(operand);
            break;
        case '7':bufs=deci(operand);
            break;
        case '8':bufs=deci(operand);
            break;
        case '9':bufs=deci(operand);
            break;
        case '#':
            operand[0]='0';
           bufs= deci(operand);
        ...
        case '$':
            bufs= hexi(operand);
            break;
     //   case default bufs= symbol(); break;
    }
    return bufs;
}
...
char *toUpText(char inText[]) { // Convert form text lower to upper etc. "heLLo" ..."HELLO"	       etc.  toUpText("hello")>>>>"HELLO"
    ...
    strcpy(obj[objindex].operand, contostr(value).buff);
...
int REL(char opc[], int n) {
    ...
    if (n == 3) {
        char strrel[5];
        strcpy(strrel, contoaddress(tmp).buff);
        char strrel1[3];
        char strrel2[3];
        strrel1[0] = strrel[0];
        strrel1[1] = strrel[1];
        ...
        strrel2[1] = strrel[3];
        strrel2[2] = '\0';
        strcpy(obj[objindex].operand, strrel1);
        strcpy(obj[objindex].operand2, strrel2);
    } else {
        printf("tmp %d  \n", tmp);
...
opcode findOpcode(char code[], char addressmode[]) {
    printf(" code :%s addrmod: %s", code, addressmode);
    opcode opc;
    FILE *fil;
    char ch0[5];
    char ch1[5];
...
int interpret(char line[]) {
    ...
    if (strcmp(str[n], "ORG") == 0) {
        ...
        char tmpdb[3];
        objindex++;
        strcpy(tmpdb, str[n + 2]);
        if (tmpdb[0] == '#') {
            tmpdb[0] = '0';
            int valdb = conint(tmpdb);
            strcpy(obj[objindex].opcode, contostr(valdb).buff);
            strcpy(obj[objindex].operand, "");
            strcpy(obj[objindex].operand2, "");
            strcpy(obj[objindex].address, contoaddressvariable(PC).buff);
        }
        if (tmpdb[0] == '$') {
            tmpdb[0] = ' ';
            strcpy(obj[objindex].opcode, tmpdb);
            strcpy(obj[objindex].operand, "");
            strcpy(obj[objindex].operand2, "");
            strcpy(obj[objindex].address, contoaddressvariable(PC).buff);
        }
        ...
        if (tmpdb[0] == '$') {
            ...
            tmp3[0] = tmpdb[3];
            tmp3[1] = tmpdb[4];
            strcpy(obj[objindex].opcode, tmp2);
            strcpy(obj[objindex].operand, tmp3);
            strcpy(obj[objindex].operand2, "");
            strcpy(obj[objindex].address, contoaddressvariable(PC).buff);
        }
        PC += 2;
    ...
    } else if (ischilSymbol(str[n]))// true save to SYMBOL TABLE
    {
        ...
        opcode op = findOpcode(strtmp, isAddressMode(str[n + 1]));
        printf("opcode %s \n", op.op);
        excuteMode(op.op, op.byte);
        printf("%s address:%s %s %s %s   ; %s \n", strtmp, obj[objindex].address, obj[objindex].opcode, obj[objindex].operand, obj[objindex].operand2, obj[objindex].list);
    }
}
