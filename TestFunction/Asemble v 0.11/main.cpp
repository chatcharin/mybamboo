...
HEX initianSymbol(HEX line){
    ...
    if(line.valueopcode.byte <=2){
     ...
     char tmpaddress[5] ;
     strcpy(tmpaddress,contoaddress(pctmp).buff);
...
int inserttoLineObject(char opcode[],char operand[],char operand2[],int n, char line[]){
        objindex++;      
...
int excuteMode(HEX line) {
    if(line.MODE ==5){
        ...
        line.hexb[3]='\0';
        inserttoLineObject(line.valueopcode.op,line.hexa,line.hexb,line.valueopcode.byte,LINES);
    }else{
        line.hexb[3]='\0';
        inserttoLineObject(line.valueopcode.op,line.hexa,line.hexb,line.valueopcode.byte,LINES);
    }
...
int interpret(char line[]) {
    ...
    if (strcmp(LINE.block1, "ORG") == 0) {
        ...
        hexline = initiantVariable(hexline);
        insertoSymbolTable(indexSymbol,LINE.block1,contoaddress(PC).buff,PC,objindex++);
        inserttoLineObject(hexline.hexa,hexline.hexb,nulls,1,LINES);
        indexSymbol++;
    } else if (strcmp(LINE.block2, "DW") == 0) {
        strcpy(hexline.dataoperand,LINE.block3);
        hexline = initiantVariable(hexline);
        insertoSymbolTable(indexSymbol,LINE.block1,contoaddress(PC).buff,PC,objindex++);
        inserttoLineObject(hexline.hexa,hexline.hexb,nulls,2,LINES);
        indexSymbol++;
