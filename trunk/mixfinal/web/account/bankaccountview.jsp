...
// end   =  limit + pagedisplay
   ...
   //ตรวจสอบ username และ password ว่ามีอยู่จริงหรือไม่
    sql="select * from accountbank limit "+pages+","+limit;
    rs=stmt.executeQuery(sql);
    out.print("<table  id=\"gradient-style\" width=\"100%\">" +
              "<thead><tr>" +
              "<th scope=\"col\" >รหัสบัญชี</th>" +
              "<th scope=\"col\" >รหัสบัญชีธนาคาร</th>" +
    ...
    <%
      for(int i=0;i<30;i++){
        out.println("<tr>");
        if(rs.next()){
          out.print("<td>" + rs.getString("accountbank_id") + "</td>\n" +
                    "<td>" + rs.getString("bank_id") + "</td>\n"+
                    "<td>" + rs.getString("status") + "</td>\n"+
                    "<td>" + String.valueOf(rs.getDouble("value")) + "</td>\n"+
                    "<td>" + rs.getString("detail") + "</td>\n");
          out.print("<td><a href=\"index.jsp?pages=dataaccountbank&accountbank_id="+rs.getString("accountbank_id")+"\" >ดูข้อมูล</a></td>\n");
          out.print("<td><a href=\"index.jsp?pages=editaccountbank&accountbank_id="+rs.getString("accountbank_id")+"\" >แก้ไข</a></td>\n");
          out.print("<td><a href=\"index.jsp?pages=accountbankview&delete=1&accountbank_id="+rs.getString("accountbank_id")+"\" >ลบ</a></td>\n");
        }
       out.println("</tr>");
      }
    out.println("</tbody>");
